...
const readAnalyticsFile = (file) => {
  return new Promise((resolve) => {
    ...
    for (let eventName of eventsData.eventsEnabled)
    ...
    file.ondata = (_err, data, final) => {
      ...
      loadTask.set(
        `Loading user statistics... ${Math.ceil(
...
export const extractData = async (files) => {
  ...
  const confirmedPayments = extractedData.user.payments.filter(
    (p) => p.status === 1
  );
  if (confirmedPayments.length) {
    extractedData.payments.total += confirmedPayments
      .map((p) => p.amount / 100)
      .reduce((p, c) => p + c);
    extractedData.payments.list += confirmedPayments
      .sort(
        (a, b) =>
  ...
  const isOldPackage =
    channelsIDsFile[0].name.match(/messages\/(c)?([0-9]{16,32})\/$/)[1] ===
    undefined;
      ...
      return new Promise((resolve) => {
        ...
        Promise.all([
          ...
          if (!rawData || !rawMessages) {
            console.log(
              `[debug] Files of channel ${channelID} can't be read. Data is ${!!rawData} and messages are ${!!rawMessages}. (path=${channelDataPath})`
            );
            return resolve();
          } else messagesRead++;
          ...
          const dmUserID = isDM
            ? data.recipients.find((userID) => userID !== extractedData.user.id)
            : undefined;
          channels.push({
            data,
            messages,
  ...
  extractedData.topChannels = channels
  ...
  extractedData.characterCount = channels
  ...
  for (let wordData of extractedData.favoriteWords) {
    ...
    if (userID) {
      ...
      extractedData.favoriteWords[
        extractedData.favoriteWords.findIndex((wd) => wd.word === wordData.word)
      ] = {
  ...
  await Promise.all(
    extractedData.topDMs.map((channel) => {
      return new Promise((resolve) => {
        fetchUser(channel.dmUserID).then((userData) => {
          const channelIndex = extractedData.topDMs.findIndex(
            (c) => c.id === channel.id
          );
  ...
  const statistics = await readAnalyticsFile(
    files.find((file) =>
  ...
  extractedData.averageOpenCountPerDay =
    extractedData.openCount &&
    perDay(statistics.openCount, extractedData.user.id);
  ...
  extractedData.averageMessageCountPerDay =
    extractedData.sentMessageCount &&
    perDay(extractedData.sentMessageCount, extractedData.user.id);
