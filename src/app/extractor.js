...
export const extractData = async (files) => {
  ...
  await Promise.all(
    channelsIDs.map((channelID) => {
      return new Promise((resolve) => {
        ...
        Promise.all([readFile(channelDataPath), readFile(channelMessagesPath)]).then(([rawData, rawMessages]) => {
  ...
  const statistics = await readAnalyticsFile(
    files.find((file) => /activity\/analytics\/events-[0-9]{4}-[0-9]{5}-of-[0-9]{5}\.json/.test(file.name))
  );
  ...
  extractedData.averageMessageCountPerDay =
    extractedData.sentMessageCount && perDay(extractedData.sentMessageCount, extractedData.user.id);
  extractedData.slashCommandUsedCount = statistics.slashCommandUsedCount;

  let { data: detectables } = await axios({
    method: "GET",
