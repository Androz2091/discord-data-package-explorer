<script>
  ...
  async function handleFile(file) {
    ...
    if (!validPackage) {
      error =
        "Your package seems to be corrupted. Click or drop your package file here to retry";
      loading = false;
      return;
    }
    ...
    extractData(files)
      .then((extractedData) => {
        ...
        console.log(
          `[debug] Data extracted in ${
            (Date.now() - extractStartAt) / 1000
          } seconds.`
        );
        navigate("/stats");
      })
      .catch((err) => {
        if (err.message === "invalid_package_missing_messages") {
          error =
            'Some data is missing in your package, therefore it can not be read. <br> It is a bug on Discord side (06-10-21), and will be fixed in the next few days. <br> Join <a href="https://androz2091.fr/discord">our Discord</a> to get more information.';
          loading = false;
        } else {
          error =
  ...
  <div class="app-loader">
    <div class="app-loader-boxes">
      ...
      <p class="app-loader-description">
        <a href="/" use:link>DDPE</a> is a website that generates stats from your
        Discord Data Package. It is your device that processes the data, nothing
      ...
      <div class="app-discord">
        <a href="https://androz2091.fr/discord" target="_blank"
          ><button class="app-discord-btn"
            >Need help? Chat with us on Discord!</button
          ></a
        >
      </div>
      <div class="app-demo">
