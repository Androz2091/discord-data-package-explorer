<script>
  ...
  const hoursLabels = new Array(24)
    .fill(0)
    .map((v, i) => (i == 0 ? "12am" : i < 12 ? `${i}am` : i == 12 ? "12pm" : `${i - 12}pm`));
</script>
  ...
  {#if $data}
    <div class="cards">
        ...
        >
          <h3 slot="content">
            ...
            <span class="text-discord"
              ><SvelteTooltip
                tip="Used {$data.favoriteWords[0].count.toLocaleString('en-US')} times"
                bottom
                color="#000000"><span class="text-discord">{$data.favoriteWords[0].word}</span></SvelteTooltip
              ></span
            >
            and
            <span class="text-discord"
              ><SvelteTooltip
                tip="Used {$data.favoriteWords[1].count.toLocaleString('en-US')} times"
                bottom
                color="#000000">{$data.favoriteWords[1].word}</SvelteTooltip
              ></span
            >
          </h3>
        </FunFact>
      ...
      <Card name="hours">
        ...
        <p>
          {hoursLabels[$data.hoursValues.indexOf(Math.max(...$data.hoursValues))]} is definitely your favorite hour to chat
          with your friends!
        </p>
        <Chart
          data={{
      ...
      <Card name="third">
        ...
        /><FunFact
          svg=""
          content="You were in voice channels during %"
