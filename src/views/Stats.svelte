<script>
	...
	import FlipCardFront from "../components/FlipCardFront.svelte";
	import FlipCardBack from "../components/FlipCardBack.svelte";
	...
	{#if $data}
		<div class="cards">
			...
			<Card name="first">
				...
				<FunFact
					...
					explanation="That's about {$data.averageMessageCountPerDay &&
						$data.averageMessageCountPerDay.toLocaleString('en-US')} messages per day!"
				/>
				<FunFact
					...
					explanation="You are opening Discord ~{$data.averageOpenCountPerDay &&
						$data.averageOpenCountPerDay.toLocaleString('en-US')} times per day!"
				/>
			</Card>
			<Card name="second">
				<FunFact
						...
						<h3>
							...
							<span
								...
								on:click={$data.payments.list.length
									? showModal(`<h3 style="text-align: center">${$data.payments.list}</h3>`)
									: undefined}
							>
				...
				>
					<h3 slot="content">
						...
						<span class="text-discord"
							><SvelteTooltip tip="Used {$data.favoriteWords[0].count.toLocaleString('en-US')} times" bottom color="#000000"
								><span class="text-discord">{$data.favoriteWords[0].word}</span></SvelteTooltip
							></span
						>
						and
						<span class="text-discord"
							><SvelteTooltip tip="Used {$data.favoriteWords[1].count.toLocaleString('en-US')} times" bottom color="#000000"
			...
			<Card name="hours">
				...
				<p>
					{hoursLabels[$data.hoursValues.indexOf(Math.max(...$data.hoursValues))]} is definitely your favorite hour to chat with your friends!
				</p>
				<Chart
					data={{
						labels: hoursLabels,
			...
			<Card name="top-channels">
				<Leaderboard title="Top Channels" description="The channels you chat the most in!">
					{#each $data.topChannels as channel, i}
						<LeaderboardItem position={i} name={channel.name} guild={channel.guildName} count={channel.messageCount} channel />
					{/each}
				</Leaderboard>
			</Card>
			<Card name="top-games">
						...
						{#each users as user}
							<SvelteTooltip tip={user.username} bottom color="#000000">
								<a href={user.url} target="_blank">
									<div class="contributors-item" style="background-image: url('{user.avatar}');" />
								</a>
							</SvelteTooltip>
						{/each}
					{:catch users}
